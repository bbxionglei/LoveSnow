...
open_socket(struct socket_server* ss, struct request_open* request, struct socket_message* result) {
	...
	do
	{
		...
		if (ns == NULL) {
			xclose(sock);
			result->data = (char*)"reach ls socket number limit";
			break;
		}
...
socket_server_poll(struct socket_server* ss, struct socket_message* result, int* more) {
	for (;;) {
		...
		struct st_event* e = &ss->ev[ss->event_index++];
		struct socket_st* s = (struct socket_st*)e->s;
		if (s == NULL) {
...
#else
		n = send(ss->sendctrl_fd, (char*)& request->header[6], len + 2, 0);
#endif
		if (n < 0) {
			if (xerrno() != EINTR) {
...
#else
				n = send(s->fd, (char*)so.buffer, so.sz, 0);
#endif
			}
			else {
				union sockaddr_all sa;
...
struct socket_info*
	...
	struct socket_info* si = (struct socket_info*)ls_malloc(sizeof(*si));
