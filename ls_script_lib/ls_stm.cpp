...
// msg should alloc by ls_malloc 
static struct stm_copy*
stm_newcopy(void* msg, int32_t sz) {
	struct stm_copy* copy = (struct stm_copy*)ls_malloc(sizeof(*copy));
	copy->reference = 1;
	copy->sz = sz;
	copy->msg = msg;
...
stm_new(void* msg, int32_t sz) {
	struct stm_object* obj = (struct stm_object*)ls_malloc(sizeof(*obj));
...
int luaopen_lovesnow_stm(lua_State * L) {
	luaL_checkversion(L);
