...
local sc = require "lovesnow.socketchannel"
local socket = require "lovesnow.socket"
local cluster = require "lovesnow.cluster.core"

local config_name = ls.getenv "cluster"
local node_address = {}
local node_session = {}
local command = {}
local config = {}
...
local function open_channel(t, key)
	...
	if address == nil and not config.nowaiting then
		...
		ct.namequery = co
		ls.error("Waiting for cluster node [".. key.."]")
		ls.wait(co)
...
local function loadconfig(tmp)
	...
	for name,address in pairs(tmp) do
		if name:sub(1,2) == "__" then
			name = name:sub(3)
			config[name] = address
			ls.error(string.format("Config %s = %s", name, address))
		else
			...
			if ct and ct.namequery and not config.nowaiting then
				ls.error(string.format("Cluster node [%s] resloved : %s", name, address))
				ls.wakeup(ct.namequery)
			end
		end
	end
...
local function send_request(source, node, addr, msg, sz)
	...
	local tracetag = ls.tracetag()
	if tracetag then
		if tracetag:sub(1,1) ~= "(" then
			-- add nodename
			local newtag = string.format("(%s-%s-%d)%s", nodename, node, session, tracetag)
			ls.tracelog(tracetag, string.format("session %s", newtag))
			tracetag = newtag
		end
		ls.tracelog(tracetag, string.format("cluster %s", node))
...
function command.proxy(source, node, name)
	...
	if proxy[fullname] == nil then
		proxy[fullname] = ls.newservice("clusterproxy", node, name)
	end
	ls.ret(ls.pack(proxy[fullname]))
end
...
local function clearnamecache()
	for fd, service in pairs(cluster_agent) do
		if type(service) == "number" then
			ls.send(service, "lua", "namechange")
		end
	end
end
...
function command.register(source, name, addr)
	...
	ls.ret(nil)
	ls.error(string.format("Register [%s] :%08x", name, addr))
end

function command.queryname(source, name)
	ls.ret(ls.pack(register_name[name]))
end

function command.socket(source, subcmd, fd, msg)
	if subcmd == "open" then
		ls.error(string.format("socket accept from %s", msg))
		-- new cluster agent
		cluster_agent[fd] = false
		local agent = ls.newservice("clusteragent", ls.self(), source, fd)
		local closed = cluster_agent[fd]
		cluster_agent[fd] = agent
		if closed then
			ls.send(agent, "lua", "exit")
		...
		if subcmd == "close" or subcmd == "error" then
			...
			ls.error(string.format("socket %s %d %s", subcmd, fd, msg or ""))
		end
	end
end
...
ls.start(function()
	loadconfig()
	ls.dispatch("lua", function(session , source, cmd, ...)
