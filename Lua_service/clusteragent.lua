...
local register_name_mt = { __index =
	function(self, name)
		...
		if waitco then
			...
			local addr = ls.call(clusterd, "lua", "queryname", name:sub(2))	-- name must be '@xxxx'
			if addr then
				self[name] = addr
			end
...
local function dispatch_request(_,_,addr, session, msg, sz, padding, is_push)
	ignoreret()	-- session is fd, don't call ls.ret
	if session == nil then
		-- trace
		tracetag = addr
		return
	...
	if addr == 0 then
		...
		if addr then
			if is_push then
				ls.rawsend(addr, "lua", msg, sz)
				return	-- no response
			else
				if tracetag then
					ok , msg, sz = pcall(ls.tracecall, tracetag, addr, "lua", msg, sz)
					tracetag = nil
				else
					ok , msg, sz = pcall(ls.rawcall, addr, "lua", msg, sz)
				end
			end
		else
			ok = false
...
ls.start(function()
	...
	ls.dispatch("lua", function(_,source, cmd, ...)
		...
		elseif cmd == "namechange" then
			...
			ls.error(string.format("Invalid command %s from %s", cmd, ls.address(source)))
		end
	end)
end)
