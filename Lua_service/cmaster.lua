...
local function dispatch_slave(fd)
	...
	elseif t == 'Q' then
		...
		ls.error("Invalid slave message type " .. t)
	end
end

local function monitor_slave(slave_id, slave_address)
	local fd = slave_node[slave_id].fd
	ls.error(string.format("Harbor %d (fd=%d) report %s", slave_id, fd, slave_address))
	while pcall(dispatch_slave, fd) do end
	ls.error("slave " ..slave_id .. " is down")
...
ls.start(function()
	local master_addr = ls.getenv "standalone"
	ls.error("master listen socket " .. tostring(master_addr))
	local fd = socket.listen(master_addr)
	socket.start(fd , function(id, addr)
		ls.error("connect from " .. addr .. " " .. id)
		socket.start(id)
		local ok, slave, slave_addr = pcall(handshake, id)
		if ok then
			ls.fork(monitor_slave, slave, slave_addr)
		else
			ls.error(string.format("disconnect fd = %d, error = %s", id, slave))
			socket.close(id)
		end
	end)
end)
