...
local mc = require "lovesnow.multicast.core"
local datacenter = require "lovesnow.datacenter"

local harbor_id = ls.harbor(ls.self())
...
function command.DEL(source, c)
	...
	if node ~= harbor_id then
		ls.send(node_address[node], "lua", "DEL", c)
		return NORET
	end
	...
	if remote then
		for node in pairs(remote) do
			ls.send(node_address[node], "lua", "DELR", c)
		end
	end
	return NORET
end
...
local function remote_publish(node, channel, source, ...)
	ls.redirect(node_address[node], source, "multicast", channel, ...)
end

-- publish a message, for local node, use the message pointer (call mc.bind to add the reference)
-- for remote node, call remote_publish. (call mc.unpack and ls.tostring to convert message pointer to string)
local function publish(c , source, pack, size)
	...
	local msg = ls.tostring(pack, size)	-- copy (pack,size) to a string
	...
	for k in pairs(group) do
		...
		ls.redirect(k, source, "multicast", c , msg)
	end
end

ls.register_protocol {
	name = "multicast",
	id = ls.PTYPE_MULTICAST,
...
function command.PUB(source, c, pack, size)
	assert(ls.harbor(source) == harbor_id)
	local node = c % 256
	if node ~= harbor_id then
		-- remote publish
...
function command.SUB(source, c)
	...
	if node ~= harbor_id then
		...
		if channel[c] == nil then
			if ls.call(node_address[node], "lua", "SUBR", c) then
				return
			end
			if channel[c] == nil then
				-- double check, because ls.call whould yield, other SUB may occur.
				channel[c] = {}
				channel_n[c] = 0
			end
		end
	...
	if group[source] then
		...
		if channel_n[c] == 0 then
			...
			if node ~= harbor_id then
				...
				channel[c] = nil
				channel_n[c] = nil
				ls.send(node_address[node], "lua", "USUBR", c)
			end
		end
	end
	return NORET
...
ls.start(function()
	ls.dispatch("lua", function(_,source, cmd, ...)
