...
function multicast.new(conf)
	...
	if self.channel == nil then
		self.channel = ls.call(multicastd, "lua", "NEW")
	end
	self.__pack = conf.pack or ls.pack
	self.__unpack = conf.unpack or ls.unpack
...
function chan:publish(...)
	local c = assert(self.channel)
	ls.call(multicastd, "lua", "PUB", c, mc.pack(self.__pack(...)))
end
...
local function init()
	multicastd = ls.uniqueservice "multicastd"
