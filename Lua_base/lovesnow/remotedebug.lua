...
local injectrun = require "lovesnow.injectcode"
local table = table
local debug = debug
...
local ctx_ls = debug.getinfo(ls.start,"S").short_src	-- skip when enter this source file
...
function linehook(mode, line)
	...
	if mode ~= "line" then
		ctx.needupdate = true
		if mode ~= "return" then
			if ctx.next_mode or debug.getinfo(2,"S").short_src == ctx_ls then
				ctx.level = 1
		...
		if ctx.needupdate then
			...
			if ctx.filename == ctx_term then
				ctx_active[co] = nil
				sethook()
				change_prompt(string.format(":%08x>", ls.self()))
				return
			end
		end
...
local function watch_proto(protoname, cond)
	local proto = assert(replace_upvalue(ls.register_protocol, "proto"), "Can't find proto table")
...
function dbgcmd.s(co)
	local ctx = ctx_active[co]
	ctx.next_mode = false
	ls_suspend(co, coroutine.resume(co))
end
	...
	if func then
		local function idle()
			if raw_dispatcher then
			    ls.timeout(10,idle)	-- idle every 0.1s
			end
		end
		ls.timeout(0, idle)
	end
...
function M.start(import, fd, handle)
	...
	ls_suspend = import.suspend
	assert(raw_dispatcher == nil, "Already in debug mode")
	ls.error "Enter debug mode"
	local channel = debugchannel.connect(handle)
	raw_dispatcher = hook_dispatch(dispatcher, ls.response(), fd, channel)
end
