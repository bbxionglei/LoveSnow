...
local function resolve(content)
	if #content < DNS_HEADER_LEN then
		-- drop
		ls.error("Recv an invalid package when dns query")
		return
	end
	...
	if answers_ipv4 then
		CACHE[QTYPE.A][question.name] = { answers = answers_ipv4, ttl = ls.now() + ttl * 100 }
	end

	if answers_ipv6 then
		CACHE[QTYPE.AAAA][question.name] = { answers = answers_ipv6, ttl = ls.now() + ttl * 100 }
	end
	...
	if question.name ~= resp.name then
		ls.error("Recv an invalid name when dns query")
	end
...
local function connect_server()
	...
	dns_server.fd = fd
	ls.error(string.format("Udp server open %s:%s (%d)", dns_server.address, dns_server.port, fd))
end

local DNS_SERVER_RETIRE = 60 * 100
local function touch_server()
	dns_server.retire = ls.now()
	...
	local function check_alive()
		if ls.now() > dns_server.retire + DNS_SERVER_RETIRE then
			local fd = dns_server.fd
			if fd then
				dns_server.fd = nil
				socket.close(fd)
				ls.error(string.format("Udp server close %s:%s (%d)", dns_server.address, dns_server.port, fd))
			end
		else
			ls.timeout( 2 * DNS_SERVER_RETIRE, check_alive)
		end
	end
...
local function lookup_cache(name, qtype, ignorettl)
	...
	if result then
		if ignorettl or (result.ttl > ls.now()) then
...
local function suspend(tid, name, qtype)
	...
	ls.fork(function()
		...
		if req then
			-- cancel tid
			ls.error(string.format("DNS query %s timeout", name))
