...
local dump = require "lovesnow.datasheet.dump"
...
local function monitor(pointer)
	ls.fork(function()
		ls.call(address, "lua", "collect", pointer)
...
function builder.new(name, v)
	...
	ls.call(address, "lua", "update", name, pointer)
...
function builder.update(name, v)
	...
	ls.send(address, "lua", "release", lp)
	dataset[name] = diff
	monitor(pointer)
end
...
function datasheet.monitor(source, handle)
	...
	if t.handle ~= handle then	-- already changes
		ls.ret(ls.pack(t.handle))
	else
		assert(not t.monitor[source])
		t.monitor[source]=ls.response()
	end
end
