	...
	while self.__sock do
		...
		if ok and session then
			...
			if co then
				...
				self.__thread[session] = nil
				ls.error("socket: unknown session :", session)
			end
		else
...
local function push_response(self, response, co)
	if self.__response then
		...
		if self.__wait_response then
			ls.wakeup(self.__wait_response)
...
local function connect_backup(self)
	if self.__backup then
		for _, addr in ipairs(self.__backup) do
			...
			ls.error("socket: connect to backup host", host, port)
...
local function connect_once(self)
			...
			if id == self.__sock[1] then
				if size == 0 then
					...
					if not self.__overload then
						...
						ls.error(string.format("WARNING: %d K bytes need to send out (fd = %d %s:%s)", size, id, self.__host, self.__port))
					end
				end
			end
		end

		ls.fork(overload_trigger, fd, 0)
	...
	self.__dispatch_thread = ls.fork(function()
	...
	if self.__auth then
		...
		if not ok then
			...
			if message ~= socket_error then
				...
				ls.error("socket: auth failed", message)
			end
		end
		self.__authcoroutine = false
...
local function try_connect(self , once)
	...
	while not self.__closed do
		...
		if ok then
			if not once then
				ls.error("socket: connect to", self.__host, self.__port)
			end
			return
		elseif once then
			return err
		else
			ls.error("socket: connect", err)
		end
		if t > 1000 then
			ls.error("socket: try to reconnect", self.__host, self.__port)
			ls.sleep(t)
			t = 0
		else
			ls.sleep(t)
...
local function check_connection(self)
	if self.__sock then
		if socket.disconnected(self.__sock[1]) then
			-- closed by peer
			ls.error("socket: disconnect detected ", self.__host, self.__port)
			close_channel_socket(self)
			return
		end
...
local function block_connect(self, once)
	...
	if r == nil then
		ls.error(string.format("Connect to %s:%d failed (%s)", self.__host, self.__port, err))
		error(socket_error)
	else
		return r
	end
