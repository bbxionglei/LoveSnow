local driver = require "lovesnow.socketdriver"
local ls = require "lovesnow"
local ls_core = require "lovesnow.core"
local assert = assert
...
-- read ls_socket.h for these macro
-- LS_SOCKET_TYPE_DATA = 1
socket_message[1] = function(id, size, data)
	local s = socket_pool[id]
	if s == nil then
		ls.error("socket: drop package from " .. id)
	...
	if rrt == "number" then
		...
		if s.buffer_limit and sz > s.buffer_limit then
			ls.error(string.format("socket buffer overflow: fd=%d size=%d", id , sz))
...
-- LS_SOCKET_TYPE_ERROR = 5
socket_message[5] = function(id, _, err)
	local s = socket_pool[id]
	if s == nil then
		ls.error("socket: error on unknown", id, err)
		return
	end
	if s.connected then
		ls.error("socket: error on", id, err)
...
-- LS_SOCKET_TYPE_UDP = 6
socket_message[6] = function(id, size, data, address)
	local s = socket_pool[id]
	if s == nil or s.callback == nil then
		ls.error("socket: drop udp package from " .. id)
		driver.drop(data, size)
		return
	end
	local str = ls.tostring(data, size)
	ls_core.trash(data, size)
...
local function default_warning(id, size)
	...
	ls.error(string.format("WARNING: %d K bytes need to send out (fd = %d)", size, id))
end

-- LS_SOCKET_TYPE_WARNING
...
function socket.shutdown(id)
	...
	if s then
		driver.clear(s.buffer,buffer_pool)
		-- the framework would send LS_SOCKET_TYPE_CLOSE , need close(id) later
		driver.shutdown(id)
	end
end
...
function socket.close(id)
	...
	if s.connected then
		...
		-- because ls.wait never return in __gc, so driver.clear may not be called
		if s.co then
